...
export class ProjectDocListClient {
     ...
     * @param projectID Full Project ID
     * @param forDocType Doc Type Key
     * @param currentState KVP of current row keys and etags
     */
    getChangedDocList(projectID: string, forDocType: string, currentState: CurrentDataState[]) {
        return new Promise<DataDifferential | null>((resolve, reject) => {
            this.getChangedDocListWithCallbacks(projectID, forDocType, currentState, (result) => resolve(result), (exception, _reason) => reject(exception));
        });
    }

    private getChangedDocListWithCallbacks(projectID: string, forDocType: string, currentState: CurrentDataState[], onSuccess?: (result: DataDifferential | null) => void, onFail?: (exception: string, reason: string) => void) {
        ...
        const content_ = JSON.stringify(currentState);
...
export class DataDifferential implements IDataDifferential {
    ...
    Add?: ModelBase[] | undefined;
    /** Collection of rows with new etags (replace) */
    Change?: ModelBase[] | undefined;
        ...
        if (_data) {
            if (Array.isArray(_data["Add"])) {
                ...
                for (let item of _data["Add"])
                    this.Add!.push(ModelBase.fromJS(item));
            }
            if (Array.isArray(_data["Change"])) {
                this.Change = [] as any;
                for (let item of _data["Change"])
                    this.Change!.push(ModelBase.fromJS(item));
            }
...
/** Used by Most DataModel classes */
...
export class CurrentDataState implements ICurrentDataState {
    /** Key of Row/object */
    RowKey!: string;
    /** ETag of Row/object  */
    ETag!: string;

    constructor(data?: ICurrentDataState) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): CurrentDataState {
        data = typeof data === 'object' ? data : {};
        let result = new CurrentDataState();
        result.init(data);
        return result;
    }
