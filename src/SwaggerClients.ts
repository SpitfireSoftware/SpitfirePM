...
export class ProjectTeamClient {
     ...
     * @param teamMemberKey ID of team member entity to replace, not the UserKey!
    ...
    private updateTeamMemberWithCallbacks(projectID: string, teamMemberKey: string, newData: ProjectTeamMember, onSuccess?: (result: string) => void, onFail?: (exception: string | string | string | string, reason: string) => void) {
        let url_ = this.baseUrl + "/api/Project/{projectID}/Team/{teamMemberKey}";
        if (projectID === undefined || projectID === null)
            throw new Error("The parameter 'projectID' must be defined.");
        url_ = url_.replace("{projectID}", encodeURIComponent("" + projectID));
